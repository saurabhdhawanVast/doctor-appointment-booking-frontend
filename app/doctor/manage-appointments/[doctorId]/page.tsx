...
const DoctorSchedulePage: React.FC<{ params: { doctorId: string } }> = ({
  ...
  const [slots, setSlots] = useState<any[]>([]); // State to store slots for the selected date
  ...
  useEffect(() => {
    if (selectedDate) {
      ...
      const dateObj = availableDates.find(
        (avail) =>
          new Date(avail.date).toISOString().split("T")[0] === formattedDate
      );
      if (dateObj) {
        setSlots(dateObj.slots);
        setShowSlots(true);
      } else {
        setSlots([]);
        setShowSlots(false);
  ...
  const handleCancelSlot = (slotId: string) => {
    if (selectedDate) {
      const formattedDate = selectedDate.toISOString().split("T")[0];
      // Assuming cancelSlot requires doctorId, date, and slotId
      cancelSlot(params.doctorId, formattedDate, slotId);
    }
  };
  ...
  const handleCancelAllSlots = () => {
    if (selectedDate) {
      const formattedDate = selectedDate.toISOString().split("T")[0];
      // Assuming cancelAllSlots requires doctorId and date
      cancelAllSlots(params.doctorId, formattedDate);
    }
  };
  ...
  const handleAddDate = () => {
    if (selectedDate) {
      const formattedDate = selectedDate.toISOString().split("T")[0];
      toggleAvailableDate(formattedDate);
    }
  };
      ...
      <div className="w-1/4 border-r">
        ...
        <DatePicker
          ...
          dayClassName={(date) => {
            ...
            return isAvailable ? "bg-green-500 text-white" : "bg-white"; // Ensure default string is returned
          }}
        />
        <button
      ...
      <div className="w-3/4 p-4">
        {selectedDate && showSlots && (
          <div>
            ...
            <div>
              {slots.map((slot) => (
                <div key={slot.id} className="border p-2 my-2">
                  <p>Time: {slot.time}</p>
        ...
        {!selectedDate && (
          <div>
            <h2>Select a Date</h2>
            <p>Pick a date to view or manage slots.</p>
          </div>
        )}
      </div>
    </div>
